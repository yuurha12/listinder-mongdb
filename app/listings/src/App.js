...
class App extends Component {
  ...
  async onSwipe(direction, listingId, sessionId) {
    ...
    if (direction === "left") {
      await axios.delete(`http://localhost:6681/listings/delete/${listingId}`)
    } else {
      await axios.post("http://localhost:6681/listings/recordSwipe", { id: listingId, session_id: sessionId, direction })
    }
  }
  ...
  async handleClick(listingId) {
    ...
    await axios.post("http://localhost:6681/listings/updateLike", { id: listingId });
  }
